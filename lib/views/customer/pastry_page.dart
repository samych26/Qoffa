...
class _PastryPageState extends State<PastryPage> {
  ...
  String _selectedType = FavoritControl.pastry_Type;
  ...
  void initState() {
    super.initState();
    _loadPastryFavorites();
  }

  Future<void> _loadPastryFavorites() async {
    setState(() {
      _isLoading = true;
    });

    try {
      final pastryFavorites = await _controller.fetchPastryFavorites();

      if (pastryFavorites.isEmpty) {
            ...
            Expanded(
              child:
                  _isLoading
                      ...
                      ? const Center(
                        child: Text(
                          "Aucun favori de type pastry trouvé",
  ...
  Widget _buildFavoriteCard(Map<String, dynamic> item) {
    ...
    final double note = (item['note'] ?? 0).toDouble();
            ...
            children: [
              ...
              _buildCategoryButton(
                ...
                () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (_) => RestaurantPage()),
                  );
                },
              ),
              _buildCategoryButton('assets/images/market.png', 'Market', () {
                Navigator.push(
                  context,
